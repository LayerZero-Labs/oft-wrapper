...
function getContractInheritance(baseContractString, remainingContracts, finalObj) {
    ...
    if (!contractNamesInherited) {
        return finalObj
    }

    // extract import names
    contractNamesInherited = contractNamesInherited.map((x) => {
        if (x.includes('"')) {
            return x.split(`"`)[1].split(`/`).pop()
        } else {
            return x.split(`'`)[1].split(`/`).pop()
        }
    })
...
function formatPutObj(baseContract, contractBuildInfo, contractDeployment, taskArgs, hre) {
    ...
    if (baseContract[1]["content"].includes("constructor(")) {
        let constructorTypes = contractDeployment["abi"]
            .filter((x) => x["type"] && x["type"] == "constructor")[0]
            ["inputs"].map((x) => x["type"])
